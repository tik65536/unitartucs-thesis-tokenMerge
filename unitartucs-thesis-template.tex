...
As described in \ref{incomplearning}, one of the methods to simulate incomplete language is by merging specific tokens which is done by Conv1D. The merge block also comes with 2 parameters($\alpha$,$\beta$) that use to specify the minimum/maximum token to merge and the maximum token to merge accordingly. These parameters will be used to generate a list of merge sizes randomly. Here is an example on the list generated($\alpha=2$,$\beta=3$) :
\begin{center}
$l$=[2,3,3,2,3,2]    
\end{center}

After tokens pass through the embedding layer, it will result in a tensor size of NxE (N=sequence length, E=embedding dim). This matrix will then transform to a tensor of size NxEx$\beta$ 


\subsection{Full Model Architecture}
The model used in the experiment is a 3-stacking layer of LSTM network with input from an embedding layer followed by a fully connected layer for output classification. 

Table() and figure() shows a detail description on the model 
