...
The merge tensor $\mathbb{M}$ will be used as input to Conv1D(k=2,stride=1) so that each slice of $\mathbb{M}$ is transformed to the size of (E,1). The following summarized the size transformation from embedding until merge:
\begin{center}
(N, E) \rightarrow (N,E,$\beta$) \rightarrow (N,E,1)  
\end{center}
