...
\caption{Trick the model}
\label{fig:alwayswrong}
\end{center}
\end{figure}

\subsubsection{Token Merge}
The concept of merge tokens origin from the MERGE operator(\ref{introLanguage}) of bare phrase structures, as introduced above, MERGE help to build a structural description set from sentences and assume to take 2 input. This experiment, it is trying to simulate the MERGE in 3 ways:
\begin{center}
\begin{enumerate}
    \item Random Merge
...
\end{enumerate}
\end{center}
Again, the specific position in the second way is relying on POS tag, for example, merge tokens that must include ADJ, on the other hand, the third way is to merge tokens in a specific ordered pattern such as ADJ+NOUN. The details of merge will be described in \ref{modelMerge}. 
...
The architecture for the model consists of 4 main blocks which are Embedding, Merge, LSTM, and classification. The following table(\ref{tab:modelsettings}) and diagram(\ref{fig:modelArch}) give a summary and visualization of the parameters set for those blocks:
\begin{table}[!h]
\begin{tabular}{|c|l|}
\hline
...
Always Wrong                                                         & \multicolumn{1}{c|}{NOUN, ADJ, PROPN , ADJ+NOUN, NOUN+VERB+ADJ} \\ \hline
