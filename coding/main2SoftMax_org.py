...
def CellStateSimility(cellstate,nidx,pidx,weight):
    ...
    n=torch.abs(torch.triu(torch.mm(negCellState,negCellState.T),diagonal=1))
    p=torch.abs(torch.triu(torch.mm(posCellState,negCellState.T),diagonal=1))
    btwgroup = torch.abs(torch.triu(torch.mm(negCellState[:minlen,:],posCellState[:minlen,:].T),diagonal=1))
...
model=PredictLSTMIntervionP(n_vocab,embeddingDim,seqlen,seqlen,minmerge,maxmerge,batchsize,GRU,hiddenLayer,hiddenSize,bidirectional,True,numOfConvBlock,groupRelu,convpredict,kernelsize,1)
weightHistory={}
...
for epoch in range(epochs):
    ...
    for d in range(0,batchsize,batchsize):
        ...
        plt.cla()
        printout=f'Epoch: {epoch} Batch TrainDoc#{(d+batchsize):5d}, Switch:{int(switch):1d}, Preprocess:{int(preprocessswitch):1d}, AvgLoss:{avgloss:0.6f}, AvgAccy:{batchaccy:0.3f}, AvgAccy2:{batchaccy2:0.3f}'
    ...
    with torch.no_grad():
        ...
        for d in range(0,len(test_text),batchsize):
            ...
            for i in range(0,c,sliding):
                ...
                btwGroupSimility.append(btwgroups)
