...
parser.add_argument('-promote', nargs="*", default=[],
                    help='list of pos for promote learning')

parser.add_argument('-inhibiteps', type=float, default=0.0,
                    help='variance for inhibit loss with mean=0.5')

parser.add_argument('-promoteeps', type=float, default=0.0,
                    help='multiplier for promote loss')

parser.add_argument('-noinverse', type=int, default=1,
                    help='No inverse of loss')
...
promotelist=args.promote
neps = args.inhibiteps
peps = args.promoteeps
noinverse = 1 if(args.noinverse==1) else 0
...
for epoch in range(epochs):
    ...
    for d in range(0,batchsize,batchsize):
        ...
        for i,x in enumerate(nlp.pipe(train_text[d:d+batchsize])):
            ...
            for t in x:
                if(preprocessswitch==0):
                    ...
                    promote+= [1] if(t.pos_ in promotelist ) else [0]
                    pos+= [t.pos_]
            ...
            if(len(data)>maxlen):
                ...
                promote=(promote+([0]*(maxlen-orglen)))
        ...
        promotelists=np.array(promotelists)
        ...
        for i in range(0,c,sliding):
            ...
            promote=promotelists[:,i:i+seqlen]
            inhibit=inhibitlists[:,i:i+seqlen]
            poslist = tokenpos[:,i:i+seqlen]
            if(not carryforward):
                 state=model.init_state()
            pred, state ,mergeidx =  model(sequence, state, switch,permuteidx,onlyMerge,poslist)
            poslist = poslist[:,permuteidx]
            endidx = np.where(poslist=='e0s')[0]
            endidx = np.in1d(range(poslist.shape[0]),endidx)
            selectidx = np.arange(len(poslist))[~endidx]
            if(len(mergeidx)==batchsize):
                totalpos=[]
                for idx in selectidx:
                    for midx in [ m for m in mergeidx[idx] if(len(m)>1) ]:
                        midx.sort()
                        pos = '_'.join(poslist[idx,midx])
                        totalpos.append(pos)
                mergeStatistic.update(totalpos)
            else:
                poslist=poslist[selectidx,:]
        ...
        plt.cla()
        printout=f'Epoch: {epoch} Batch TrainDoc#{(d+batchsize):5d}, Switch:{int(switch):1d}, Preprocess:{int(preprocessswitch):1d}, AvgLoss:{avgloss:0.4f}, AvgAccy:{batchaccy:0.3f}, AvgAccy2:{batchaccy2:0.3f}'
    ...
    print(f'Epoch: {epoch:2d} Training Finished, AvgTrainingLoss:{avgtrainingloss:0.6f}, AvgAccy:{avgtrainingAccy:0.3f}, Time:{(e-s):2.4f}',flush=True)
    ...
    with torch.no_grad():
        ...
        for d in range(0,len(test_text),batchsize):
            ...
            for i in range(0,c,sliding):
                ...
                #            pos = '_'.join(poslist[idx,midx])
                #            totalpos.append(pos)
                #    valmergeStatistic.update(totalpos)
                #else:
                #    poslist=poslist[selectidx,:]
                #    for midx in [ m for m in mergeidx if(len(m)>1) ]:
                #        pos = poslist[:,midx]
                #        pos=[ "_".join(pos[pidx]) for pidx in range(len(pos))  ]
                #        valmergeStatistic.update(pos)
                loss=valcriterion(pred,t)
                losses.append(loss.item())
                pred=torch.nn.functional.softmax(pred,dim=1)
                pred=pred.permute(0,2,1).detach().cpu().numpy()
            ...
            [ axes[i].clear() for i in range(4) ]
            plt.cla()#
            print(f'Epoch: {epoch:2d} Batch ValDoc#{(d+batchsize):5d}, Switch:{int(switch):1d}, Preprocess:{int(preprocessswitch):1d}, AvgLoss:{avgloss:0.3f}, AvgAccy:{batchaccy:0.3f}, AvgAccy2:{batchaccy2:0.3f}, NS:({avgNegSimiality:0.3f},{avgNegSimialityMedian:0.3f}), PS:({avgPosSimiality:0.3f},{avgPosSimialityMedian:0.3f}), btwGroup:({avgbtwGroupSimiality:0.3f},{avgbtwGroupSimialityMedian:0.3f})',flush=True)
            valbatchcount+=1
    ...
    print(f'Epoch: {epoch:2d} Validation Finished, Avg Loss:{avgValloss:0.6f}, AvgAccy:{valAvgAccy:0.3f}, AvgAccy2:{valAvgAccy2:0.3f}, AvgNS:({valNS:0.3f},{valNSM:0.3f}), AvgPS:({valPS:0.3f},{valNS:0.3f}), AvgBTG:({valbtwS:0.3f},{valbtwGM:0.3f})',flush=True)
