...
parser.add_argument('-outseqlen', type=int, default=5,
                    help='out Seq Len for predict')
...
seqlen=args.seqlen
outseqlen=seqlen if(args.outseqlen==0 or args.outseqlen>seqlen) else args.outseqlen
...
model=PredictLSTMIntervionP(n_vocab,embeddingDim,seqlen,outseqlen,minmerge,maxmerge,batchsize,GRU,hiddenLayer,hiddenSize,bidirectional,True,numOfConvBlock,groupRelu,convpredict,kernelsize,1)
weightHistory={}
...
for epoch in range(epochs):
    ...
    for d in range(0,batchsize,batchsize):
        ...
        for i,x in enumerate(nlp.pipe(train_text[d:d+batchsize])):
            ...
            tmp=np.zeros(shape=maxlen if(outseqlen==seqlen) else maxlen//outseqlen)
            c=5 if(seqlen>=maxlen) else seqlen
            c=seqlen if(outseqlen==seqlen) else outseqlen
            tmp[:c]=2
            ...
            if(len(data)>maxlen):
                ...
                orglen=len(data)
                idx=orglen-1
                idx//=1 if(outseqlen==seqlen) else outseqlen
        ...
        for i in range(0,c,sliding):
            ...
            target=targets[:,targetidx:targetidx+outseqlen]
            if(not carryforward):
                 state=model.init_state()
            pred, state, tokensegement =  model(sequence, state, switch)
            #pred=pred.reshape(batchsize,seqlen,3) # N,L,1 -> L,1 for batchsize == 1
            loss= criterion(pred,target)
            losses.append(loss.item())
            if(tokensegement is not None): tokendist.append(tokensegement[:seqlen].detach().cpu().numpy())
            trainloss.append(loss.item())
            pred=torch.nn.functional.softmax(pred,dim=1)
            pred=pred.permute(0,2,1).detach().cpu().numpy()
            predict_history[:,targetidx:targetidx+outseqlen,:]=pred[:batchsize]
            if(GRU==False):
        ...
        est_magnitude=[]
        drawcount=0
        maxidx= [ np.argmax(predict_history[i, idxarray[i]-(20//outseqlen):idxarray[i]-1, :],axis=-1) for i in range(batchsize)]
        for i,each in enumerate(maxidx):
            if(each.shape[0]==0):
                if(idxarray[i]<20):
                    maxidx[i]=np.argmax(predict_history[i, idxarray[i]-(5//outseqlen):idxarray[i]-1, :],axis=-1)
                    drawcount+=1
        ...
        printout=f'Epoch: {epoch} Batch Training Doc#{(d+batchsize):5d}, Switch:{int(switch):1d}, Preprocess:{int(preprocessswitch):1d}, AvgLoss:{avgloss:0.6f}, AvgAccy:{batchaccy:0.3f}, AvgAccy2:{batchaccy2:0.3f} Magnitude:{batchmagnitude:0.3f}, DrawCount:{drawcount:2d}'
    ...
    with torch.no_grad():
        ...
        for d in range(0,len(test_text),batchsize):
            ...
            for i in range(0,c,sliding):
                ...
                pred=torch.nn.functional.softmax(pred,dim=1)
                pred=pred.permute(0,2,1).detach().cpu().numpy()
                predict_history[:,targetidx:targetidx+outseqlen,:]=pred
                targetidx+=outseqlen
            avgloss=np.mean(losses)
            ...
            print(f'Epoch: {epoch:2d} Batch Validation Doc#{(d+batchsize):5d}, Switch:{int(switch):1d}, Preprocess:{int(preprocessswitch):1d}, AvgLoss:{avgloss:0.6f}, AvgAccy:{batchaccy:0.3f}, AvgAccy2:{batchaccy2:0.3f}, Magnitude:{batchmagnitude:0.3f}, Drawcount:{drawcount:2d}',flush=True)
            valbatchcount+=1
