...
class LSTMCellP2(nn.Module):
    def __init__( self, wordsize,embeddingDim,seqlen,outseqlen,minMerge,maxMerge,batchsize,GRU,num_layers,hidden_size,bidirection=False,softmax=False,numofconv1d=1,groupRelu=1,convpredict=False,kernelsize=2,mergeRate=2):
        super(LSTMCellP2, self).__init__()
        ...
        if(self.GRU==False):
            for layer in range(self.num_layers):
                self.lstmcell.append(nn.LSTMCell(self.embedding_dim, self.lstm_size).to(self.device))
        else:
            self.RNN = nn.GRU(
    ...
    def forward(self, x, state,switch=0,permuteidx=None,onlyMerge=None,poslist=None,consecutive=False):
        ...
        for seq in range(self.seqlen):
            in_seq = lstminput[:,seq,:]
            h,c = h_state[0],c_state[0]
            for layer in range(self.num_layers):
                h,c=self.lstmcell[layer](in_seq,(h,c))
                h_state[layer],c_state[layer]=h,c
                in_seq = h
        o = self.predict(h_state[-1]) # o.shape = N*L*1
        o=o.permute(0,2,1)
