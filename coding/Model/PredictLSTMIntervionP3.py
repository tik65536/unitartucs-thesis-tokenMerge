    ...
    def forward(self, x, state,switch=0,permuteidx=None,onlyMerge=None,poslist=None,consecutive=False):
        ...
        if(switch==0):
            ...
            if(len(onlyMerge)==0):
                ...
                tmp[b,s,m,:]=0
                midx=torch.where(torch.count_nonzero(tokenidx,dim=-1)>1)[0]
            # Merge By Con1D in backward direction => LSTM
            for i in range(self.batchsize):
                if(len(onlyMerge)>0):
                    midx=torch.where(torch.count_nonzero(tokenidx[i],dim=-1)>1)[0]
                    lstminput[i,midx,:,:]=self.mergeConv1D(tmp[i,midx,:,:])
                    lstminput[i,~midx,:,:]=tmp[i,~midx,:,0].reshape(len(~midx),self.embedding_dim,1)
                else:
