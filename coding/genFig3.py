...
accy=pd.read_csv('./ResultLog/Curl_MergeNOUN_NewR1_val.csv')
...
path='/home/dick/RunpodData/Curl_MergeNOUN_NewR1_seqlen25_sldie25_batch1000_opt25_dynamicOptFalse_train15000_ksize2_e3_BiDirectionTrue_HL3_HS3_P1.0_MaxMerge3_MinMerge2_preprocess1.0_20230809-063952'
...
for file in curdataList:
    with open(f'{file}','rb') as f:
        try:
            ...
            pass
predictList = glob.glob(f'{path}/EstPred_*.plk')
...
with pymp.Parallel(1) as p:
    for epoch in p.range(10,lastepoch+1):
        ...
        relativePosition=np.zeros((1000,175))
        for i in range(175):
            rp=np.ceil((i/test1.loc[:]['length'])*100)
            relativePosition[:,i]=rp
        curldiff=[]
        curl_unit_diff=[]

        for i in range(0,174):
            curldiff.append(epoch_curl[:,i+1,:]-epoch_curl[:,i,:])
            curl_unit_diff.append(epoch_curl_unit[:,i+1,:]-epoch_curl_unit[:,i,:])
        curldiff=np.array(curldiff)
        curldiff=np.moveaxis(curldiff,0,1)
        curldiff_norm=np.linalg.norm(curldiff,axis=-1)
        curldiff_unit=curldiff/curldiff_norm.reshape(epoch_curl.shape[0],174,1)


        curl_unit_diff=np.array(curl_unit_diff)
        curl_unit_diff=np.moveaxis(curl_unit_diff,0,1)
        curl_unit_diff_norm=np.linalg.norm(curl_unit_diff,axis=-1)
        curl_unit_diff_unit=curl_unit_diff/curl_unit_diff_norm.reshape(epoch_curl.shape[0],174,1)

        prediction=predict[:,epoch]
        prediction=np.where(prediction>0.95,1,prediction)
        prediction=np.where(prediction<0.05,0,prediction)
        relativePosition2=np.where(relativePosition>100,-1,relativePosition)
        curldiffN=curl_unit_diff_unit
        negcolormap = cm.bwr
        co=np.zeros((1000,175))
        co[np.where(prediction==1)]=1
        norm = Normalize()
        norm.autoscale(co)
        nco=negcolormap(norm(co))
        r,c=np.where(relativePosition2[:,:174]==-1)
        nco[r,c,-1]=0
        r,c=np.where((poslist[:,:174]==70)|(poslist[:,:174]==78))
        nco[r,c,-1]=0
        r,c=np.where((curl_unit_diff_norm<0.25))
        nco[r,c,-1]=0
        ...
        for i in range(start,stop):
            keep=np.where((relativePosition2[:,i]>0) & (relativePosition2[:,i]<100))[0]
            curlax.quiver(epoch_curl_unit[keep,i:i+1,0], epoch_curl_unit[keep,i:i+1,1], epoch_curl_unit[keep,i:i+1,2],
                  curldiffN[keep,i:i+1,0],curldiffN[keep,i:i+1,1], curldiffN[keep,i:i+1,2] ,
                  length=0.05, normalize=True,color=nco[keep,i:i+1],linewidth=0.5)
            #curlax.scatter3D(curldiffN[keep,i:i+1,0], curldiffN[keep,i:i+1,1],curldiffN[keep,i:i+1,2],s=0.1,c=nco[keep,i:i+1],depthshade=True)
