...
def multiRun(args,n_vocab,tok2id,id2tok,train_pd,test_pd,traincounter,reverse_traincounter,
             weightPath,tensorboardpath,filename):
    seqlen=args.seqlen
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch: {epoch:2d} Training Finished, AvgTrainingLoss:{avgtrainingloss:0.6f}, AvgAccy:{avgtrainingAccy:0.3f}, DiffPOS:{avgdiffcount:2.3f}, Time:{(e-s):2.4f}',flush=True)
...
if __name__ == "__main__":
    ...
    foldername=f"{remark}seqlen{seqlen}_sldie{sliding}_batch{batchsize}_opt{optstep}_dynamicOpt{dynamicOpt}_train{trainSize}_ksize{kernelsize}_e{embeddingDim}_BiDirection{bidirectional}_HL{hiddenLayer}_HS{hiddenSize}_P{interventionP}_MaxMerge{maxmerge}_MinMerge{minmerge}_preprocess{preprocess}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    weightPath=f'../LSTMWeight/{foldername}'
    tensorboardpath=f"../Tensorboard2/{foldername}"
    pool = Pool(process)
    para = [ (args,n_vocab,tok2id,id2tok,train_pd,test_pd,traincounter,reverse_traincounter,
              weightPath+f'_Run_{i}',tensorboardpath+f'_Run_{i}',f'./Output_Run_{i}') for i in range(process) ]

    pool.starmap(multiRun,para)
