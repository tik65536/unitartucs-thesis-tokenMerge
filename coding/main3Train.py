...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        writer.add_scalars(f'Validation negNorm',[{f'negblockNorm_{i}':valblocknegNorm[i]/3 for i in range(1,6)}][0],epoch)
        writer.add_scalars(f'Validation posNorm',[{f'posblockNorm_{i}':valblockposNorm[i]/3 for i in range(1,6)}][0],epoch)
        writer.add_scalars(f'Validation negSimiality',[{f'block_{i}':valBlockNegSimility[i] for i in range(1,6)}][0],epoch)
        writer.add_scalars(f'Validation posSimiality',[{f'block_{i}':valBlockPosSimility[i] for i in range(1,6)}][0],epoch)
        writer.add_scalars(f'Validation BTWSimiality',[{f'block_{i}':valBlockbtwGroupSimility[i] for i in range(1,6)}][0],epoch)
