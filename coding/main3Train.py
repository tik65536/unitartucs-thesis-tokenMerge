...
for epoch in range(epochs):
    ...
    for d in range(0,batchsize,batchsize):
        ...
        for i in range(0,c,sliding):
            ...
            if(not carryforward):
                 state=model.init_state()
            pred,_, state ,mergeidx =  model(sequence, state, switch,permuteidx,onlyMerge,poslist,consecutive)
    ...
    with torch.no_grad():
        ...
        for block in range(12):
            ndata=np.array([ valnegNorm[i][:,seqlen*block:seqlen*block+seqlen,:] for i in range(valbatchcount) ])
            pdata=np.array([ valposNorm[i][:,seqlen*block:seqlen*block+seqlen,:] for i in range(valbatchcount) ])
