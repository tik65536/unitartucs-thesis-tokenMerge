    ...
    for d in range(0,len(test_text),batchsize):
        ...
        for i in range(0,c,sliding):
            ...
            if(d<batchsize and i<(sliding*7)):
                ...
                curlax.quiver(output[:,:,0], output[:,:,1], output[:,:,2], u, v, w, color=co[(i//seqlen)] )
                divax.quiver(output[:,:,0], output[:,:,1], output[:,:,2], u2, v2, w2, color=co[(i//seqlen)] )
                backwardcurlax.quiver(output[:,:,3], output[:,:,4], output[:,:,5], bu, bv, bw, color=co[(i//seqlen)])
                backwarddivax.quiver(output[:,:,3], output[:,:,4], output[:,:,5], bu2, bv2, bw2, color=co[(i//seqlen)])
